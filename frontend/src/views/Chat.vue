      ...
      <div class="flex-1 relative flex overflow-hidden">
        ...
        <div :class="cn('md:hidden fixed inset-y-0 left-0 flex z-[70] transition-transform duration-300', isSidebarOpen ? 'translate-x-0' : '-translate-x-full')">
          <aside class="w-16 bg-[#090b11] border-r border-white/5 flex flex-col items-center py-4 gap-4">
             <div class="w-10 h-10 rounded-xl bg-primary flex items-center justify-center">🎮</div>
             <div v-for="guild in chatStore.guilds" :key="guild.id" @click="chatStore.selectedGuild = guild" class="w-10 h-10 rounded-2xl bg-slate-800 flex items-center justify-center font-black text-xs text-slate-400 cursor-pointer">
                {{ guild.name.charAt(0).toUpperCase() }}
             </div>
          </aside>
          <aside class="w-60 bg-[#0b0d14] flex flex-col p-4">
             ...
             <nav class="flex flex-col gap-2 mb-8">
                <button v-for="tab in ['chat', 'games', 'tournaments']" :key="tab" @click="navigateTo(tab); isSidebarOpen = false" :class="cn('flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-black italic uppercase transition-all', activeTab === tab ? 'bg-primary text-white' : 'text-slate-500 bg-white/5')">
                   {{ tab }}
                </button>
             </nav>
             <h4 class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 px-2">Channels</h4>
             <div class="space-y-1">
                <div v-for="channel in chatStore.channels" :key="channel.id" @click="chatStore.selectedChannel = channel; isSidebarOpen = false" class="px-3 py-2 rounded-lg text-xs font-bold text-slate-400 hover:bg-white/5 cursor-pointer">
                   # {{ channel.name }}
                </div>
             </div>
          </aside>
        ...
        <aside class="hidden md:flex w-60 flex-col bg-[#0b0d14] border-r border-white/5">
          <div class="flex-1 p-4 overflow-y-auto">
             <h4 class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 px-2">Channels</h4>
        ...
        <div class="flex-1 flex flex-col bg-[#0f111a] relative min-w-0">
           ...
           <div v-else class="flex-1 flex flex-col">
              <div ref="messagesContainer" class="flex-1 overflow-y-auto px-4 md:px-8 py-6 flex flex-col gap-4 custom-scrollbar">
                <div v-for="(msg, i) in chatStore.messages" :key="i" class="flex gap-4 p-2 rounded-xl hover:bg-white/5 transition-all">
                  <div class="w-10 h-10 rounded-xl bg-slate-800 flex-shrink-0 flex items-center justify-center text-sm font-black border border-white/5">{{ msg.username.charAt(0).toUpperCase() }}</div>
                  <div class="flex-1 min-w-0">
                    <div class="flex items-baseline gap-2 mb-1">
                      <span class="text-xs font-black italic uppercase text-primary">{{ msg.username }}</span>
                      <span class="text-[8px] text-slate-600 font-bold">{{ new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }}</span>
                    </div>
              ...
              <div class="p-4 md:p-6 pt-0">
                 <div :class="cn('flex items-center gap-3 bg-black/40 border p-2 md:p-3 rounded-2xl transition-all', !chatStore.selectedChannel ? 'border-red-500/20 opacity-80' : 'border-white/5 focus-within:border-primary/50')">
                    <input v-model="newMessage" @keyup.enter="sendMessage" type="text" :placeholder="chatStore.selectedChannel ? 'Transmission intel...' : '❌ Select channel...'" class="flex-1 bg-transparent border-none outline-none text-sm font-bold placeholder:text-slate-700 h-10" :disabled="!chatStore.selectedChannel" />
                    <button @click="toggleVoiceChannel" :class="cn('p-2 rounded-xl transition-all', voiceStore.isInVoice ? 'bg-emerald-600/20 text-emerald-400' : 'text-slate-400 hover:text-white')">
                       {{ voiceStore.isInVoice ? '🎙️' : '🔇' }}
                    </button>
                    <button @click="sendMessage" :disabled="!chatStore.selectedChannel || !newMessage.trim()" class="px-6 py-2 bg-primary hover:bg-primary/80 disabled:opacity-50 rounded-xl text-[10px] font-black italic uppercase shadow-xl transition-all active:scale-95">SEND</button>
                 </div>
              </div>
           </div>
        </div>
        ...
        <aside class="hidden lg:flex w-64 bg-[#090b11] border-l border-white/5 p-6 flex-col overflow-y-auto">
           <h4 class="text-[9px] font-black text-slate-600 uppercase tracking-widest px-2 mb-4 italic leading-none">Tactical Units — {{ chatStore.onlineUsers.length }}</h4>
           <div class="space-y-1">
              <div v-for="u in chatStore.onlineUsers" :key="u.id" class="flex items-center gap-3 p-2 rounded-xl hover:bg-white/5 transition-all">
                <div class="relative">
                   <div class="w-8 h-8 rounded-full bg-slate-900 border border-white/5 flex items-center justify-center text-[10px] font-black uppercase text-slate-400">{{ u.username.charAt(0) }}</div>
                   <div class="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-500 rounded-full border-2 border-[#090b11]"></div>
                </div>
                <span class="text-[11px] font-black italic uppercase truncate">{{ u.username }}</span>
              </div>
           </div>
        </aside>
      </div>
